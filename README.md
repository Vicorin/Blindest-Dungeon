...
* Base game: fully accessible, some interface components and/or controls may change to create a smoother experience. (e.g. improving character sheet navigation, auditing tooltips for accuracy and readability, etc.)
* Shieldbreaker, Musketeer: Fully accessible
* The Color of  Madness: Partly accessible. I have completed the first quest using the mod, but have not yet tested the full endless mode.
* Crimson Court: Partly accessible. Some content has been supported and some should be accessible by default. It has not been thoroughly playtested though, so there are likely gaps.
(* Butcher's Circus: Not accessible yet. This is a PvP mode. The main menu for the circus is read, but it is the most unfinished area of the mod and is lower priority than other DLC.
* Localization: Incomplete. In several places, the screen-reader output had to be manually edited, due to a lack of localized text from the game. This is an active work in progress and a full localization table will be included with the mod's full release.

## Installation

1. Download [Darkest Dungeon](https://store.steampowered.com/app/262060/Darkest_Dungeon/) and run the game at least once.
2. Download and extract the mod files. Found in the releases for this repository.
3. Locate your game files. Inside the main directory, go to Windows > Win64, where you should see the Darkest.exe. Paste the DDAcess.DLL, the NVDA Controller. DLL, Tolk.DLL, and the Darkest Access launcher into that folder, alongside the game's normal exe.
4. Run Darkest Access to launch the game. After several seconds, you should hear "Accessibility mod loaded", which indicates the mod installed correctly and is running. You can create a desktop shortcut for the launcher or modify your Steam launch settings so you don't have to go into this folder every time to run the game.
